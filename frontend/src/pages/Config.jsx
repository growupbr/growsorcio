...
import { getDefaultMessage, getDefaultClosingMessage, buildWhatsappMessage } from '../utils/whatsapp';
...
const VARIAVEIS = [
  { label: '{nome}',         desc: 'Primeiro nome do lead' },
  { label: '{tipo_bem}',     desc: 'Tipo de bem (imóvel, veículo...)' },
  { label: '{valor_carta}',  desc: 'Valor da carta formatado' },
  { label: '{nome_corretor}',desc: 'Seu primeiro nome' },
  { label: '{empresa}',      desc: 'Nome da sua empresa' },
];

const PREVIEW_LEAD   = { nome: 'João Silva', tipo_de_bem: 'imóvel', valor_da_carta: 85000 };
const PREVIEW_USER   = { user_metadata: { full_name: 'Carlos Souza' } };
const PREVIEW_ORG    = 'Consórcio Elite';

// ─── Editor de template de mensagem ─────────────────────────────────────────

function MessageTemplateEditor({ etapa, onSalvar }) {
  const [aberto, setAberto] = useState(false);
  const [template, setTemplate] = useState(etapa.message_template || '');
  const [salvando, setSalvando] = useState(false);
  const [sucesso, setSucesso] = useState(false);
  const textareaRef = useRef(null);

  // Sincroniza quando a etapa mudar externamente (ex: reload)
  useEffect(() => {
    setTemplate(etapa.message_template || '');
  }, [etapa.message_template]);
  ...
  function inserirVariavel(variavel) {
    ...
    const novoTexto = template.slice(0, start) + variavel + template.slice(end);
    setTemplate(novoTexto);
    setTimeout(() => {
      el.focus();
  ...
  async function handleSalvar() {
    ...
    setSalvando(true);
    try {
      await onSalvar(etapa.id, { message_template: template.trim() || null });
      setSucesso(true);
  ...
  async function handleRestaurar() {
    ...
    setSalvando(true);
    try {
      await onSalvar(etapa.id, { message_template: null });
      setTemplate('');
      setSucesso(true);
      setTimeout(() => setSucesso(false), 2500);
    } finally {
  ...
  const templateEfetivo = template.trim() || getDefaultMessage(etapa.display_order ?? 0);
  const preview = buildWhatsappMessage(templateEfetivo, PREVIEW_LEAD, PREVIEW_USER, PREVIEW_ORG);
  const temPersonalizado = !!etapa.message_template;
  ...
  return (
    <div className="border-t border-white/5">
      ...
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-2.5 text-[11px] font-medium text-zinc-500 hover:text-zinc-300 transition-colors cursor-pointer"
      >
        <span className="flex items-center gap-2">
          <svg viewBox="0 0 24 24" fill="currentColor" className="w-3 h-3 text-emerald-500 flex-shrink-0">
      ...
      {aberto && (
        <div className="px-4 pb-4 space-y-3">
          ...
          <textarea
            ...
            placeholder={getDefaultMessage(etapa.display_order ?? 0)}
            rows={5}
            className="w-full bg-zinc-950 border border-white/8 text-zinc-100 text-xs rounded-xl px-3.5 py-2.5
                       placeholder-zinc-700 resize-none focus:outline-none focus:border-orange-500/40
                       focus:ring-1 focus:ring-orange-500/20 transition-all leading-relaxed"
          />

          {/* Chips de variáveis */}
          <div>
            <p className="text-[10px] text-zinc-600 mb-1.5 font-medium uppercase tracking-wide">Variáveis — clique para inserir</p>
            <div className="flex flex-wrap gap-1.5">
              {VARIAVEIS.map(({ label, desc }) => (
                <button
                  key={label}
                  title={desc}
                  onClick={() => inserirVariavel(label)}
                  className="px-2 py-1 rounded-lg text-[10px] font-mono font-semibold cursor-pointer
                             transition-colors bg-zinc-800 text-zinc-400 border border-white/8
          ...
          <div className="rounded-xl bg-emerald-500/5 border border-emerald-500/15 px-3 py-2.5">
            <p className="text-[10px] text-emerald-600 font-bold uppercase tracking-wide mb-1.5">Preview</p>
            <p className="text-xs text-zinc-400 leading-relaxed whitespace-pre-wrap">{preview}</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold
                         bg-orange-500 hover:bg-orange-400 disabled:opacity-50 text-white transition-all cursor-pointer"
            >
              {salvando ? '...' : sucesso ? '✓ Salvo!' : 'Salvar mensagem'}
            </button>
            {temPersonalizado && (
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-xs font-medium text-zinc-500
                           hover:text-zinc-300 hover:bg-zinc-800 disabled:opacity-50 transition-all cursor-pointer"
              >
                Restaurar padrão
              </button>
            )}
...
// ─── Seção de mensagem de encerramento ──────────────────────────────────────

function ClosingMessageSection() {
  const [template, setTemplate] = useState('');
  const [carregando, setCarregando] = useState(true);
  const [salvando, setSalvando] = useState(false);
  const [sucesso, setSucesso] = useState(false);
  const [temPersonalizado, setTemPersonalizado] = useState(false);
  const textareaRef = useRef(null);

  useEffect(() => {
    api.getOrgSettings()
      .then(({ closing_message }) => {
        setTemplate(closing_message || '');
        setTemPersonalizado(!!closing_message);
      })
      .catch(() => {})
  ...
  async function handleSalvar() {
    ...
    setSalvando(true);
    try {
      await api.updateOrgSettings({ closing_message: template.trim() || null });
      setTemPersonalizado(!!template.trim());
      setSucesso(true);
      setTimeout(() => setSucesso(false), 2500);
    } finally {
      setSalvando(false);
  ...
  async function handleRestaurar() {
    ...
    setSalvando(true);
    try {
      await api.updateOrgSettings({ closing_message: null });
      setTemplate('');
      setTemPersonalizado(false);
      setSucesso(true);
      setTimeout(() => setSucesso(false), 2500);
    } finally {
      setSalvando(false);
  ...
  const templateEfetivo = template.trim() || getDefaultClosingMessage();
  const preview = buildWhatsappMessage(templateEfetivo, PREVIEW_LEAD, PREVIEW_USER, PREVIEW_ORG);

  if (carregando) {
    return <div className="h-20 rounded-2xl bg-zinc-900 border border-white/5 animate-pulse" />;
  }
    ...
    <div className="rounded-2xl bg-zinc-900 border border-white/5 overflow-hidden">
      <div className="px-6 py-4 border-b border-white/5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-bold text-zinc-200">Mensagem de Encerramento</h2>
          {temPersonalizado && (
            <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-orange-500/15 text-orange-400 border border-orange-500/20">
              personalizada
            </span>
          )}
        </div>
        <p className="text-xs text-zinc-500 mt-1">
          Enviada quando o lead não responde. Aparece como botão discreto (X) nos cards do Kanban.
        </p>
      </div>
      ...
      <div className="px-6 py-4 space-y-3">
        <textarea
          ...
          value={template}
          onChange={(e) => setTemplate(e.target.value)}
          placeholder={getDefaultClosingMessage()}
          rows={4}
          className="w-full bg-zinc-950 border border-white/8 text-zinc-100 text-xs rounded-xl px-3.5 py-2.5
                     placeholder-zinc-700 resize-none focus:outline-none focus:border-orange-500/40
                     focus:ring-1 focus:ring-orange-500/20 transition-all leading-relaxed"
        ...
        <div>
          <p className="text-[10px] text-zinc-600 mb-1.5 font-medium uppercase tracking-wide">Variáveis</p>
          <div className="flex flex-wrap gap-1.5">
            {VARIAVEIS.map(({ label, desc }) => (
              <button
                key={label}
        ...
        <div className="rounded-xl bg-red-500/5 border border-red-500/15 px-3 py-2.5">
          <p className="text-[10px] text-red-500/70 font-bold uppercase tracking-wide mb-1.5">Preview</p>
          <p className="text-xs text-zinc-400 leading-relaxed whitespace-pre-wrap">{preview}</p>
        </div>
...
function EtapaRow({ etapa, dragHandlers, isDragging, isDropTarget, onUpdate, onDelete, onUpdateTemplate }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden bg-zinc-900 border border-white/5 hover:border-zinc-700/60 transition-all duration-150"
         style={isDragging ? { opacity: 0.4, borderColor: 'rgba(249,115,22,0.3)' } : isDropTarget ? { borderColor: 'rgba(249,115,22,0.5)', transform: 'scale(1.01)' } : {}}>
      {/* Row principal */}
      <div
        {...dragHandlers}
      ...
      <MessageTemplateEditor etapa={etapa} onSalvar={onUpdateTemplate} />
    </div>
  );
}
          ...
          <div className="space-y-2" onDragOver={(e) => e.preventDefault()}>
            {etapas.map((etapa, index) => (
              <EtapaRow
                ...
                onUpdateTemplate={handleAtualizar}
              />
            ))}
          </div>
        )}
